...
abstract class REST_Controller extends CI_Controller
{
    ...
    protected function check_app_secret_key()
    {
        $app_secret_key = $this->input->get_request_header("Appsecretkey");
        if ($app_secret_key !== $this->config->item('app_secret_key')) {
            $result = ["status" => false, "error" => "The app secret key is invalid."];
            $this->response($result);
            die;
        }
    }

    protected function check_access_token($exit = true)
    {
        if ($exit) $this->check_app_secret_key($exit);
        $rest_key_name = $this->config->item('rest_key_name');
        $access_token = trim(str_replace($rest_key_name, "", $this->input->get_request_header("Authorization")));

        $rec = get_cache_token($access_token);
        ...
        if (is_null($rec)) {
            ...
            $rec = $this->db
                ->select("employees.username,employees.employees_id,token_keys.expire_date")
                ->where("token_keys.access_token", $access_token)
                ->where("employees.sys_status", "active")
                ->where("employees.sys_action", "publish")
                ->join("employees", "employees.employees_id=token_keys.employees_id")
                ->get("token_keys")
                ->first_row('array');

            if (isset($rec['expire_date'])) {
                $expire_time = strtotime($rec['expire_date']) - strtotime(date("Y-m-d H:i:s"));
                set_cache_token($access_token, $rec, $expire_time);
            }
        }
        ...
        if (isset($rec['username'])) {
            ...
            $result = ["status" => false, "error" => "The access token is invalid."];
            if ($exit) {
                $this->response($result);
                die;
            } else {
    ...
    protected function validate_required($fields = [], $post = [])
    {
        $result = array("status" => true, "error" => "The %s is invalid.");

        foreach ($fields as $val) {
            if (!isset($post[$val]) || trim($post[$val]) == "") {
                $result["status"] = false;
                $result["error"] = sprintf($result["error"], $val);
                break;
            }
        }
    ...
    protected function check_basic_auth()
    {
        $basic_auth = trim(str_replace("Basic", "", $this->input->get_request_header("Authorization")));
        if ($basic_auth == BASIC_AUTH || $basic_auth == base64_encode(BASIC_AUTH)) {
            $result = ["status" => true];
        } else {
            $result = ["status" => false, "error" => "The access basic is invalid."];
        }
        return $result;
    }
}
