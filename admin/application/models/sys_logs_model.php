...
class Sys_logs_model extends CI_Model
{
	...
	function save_log($module = "", $action = "", $record_id = "")
	{
	...
	function get_rows($post = array())
	{
		...
		if (@$post["action"] != "") $this->db->where('sys_logs.action', $post["action"]);
		if (@$post["module"] != "") $this->db->where('sys_logs.module', $post["module"]);
		if (@$post["lastupdate_to_start"] != "") $this->db->where('DATE(sys_logs.lastupdate) >= ', setDateformat($post["lastupdate_to_start"]));
		if (@$post["lastupdate_to_end"] != "") $this->db->where('DATE(sys_logs.lastupdate) <= ', setDateformat($post["lastupdate_to_end"]));
