...
class Sys_roles extends Backend
{
	...
	function formList()
	{
		...
		$post = get_post_session('sys_roles', $post);
		...
		parent::$view_data["sorting"] = isset($post['sorting']) ? $post['sorting']  : 'sys_roles_id';
	...
	function ajaxList()
	{
		...
		if ($post) {
			set_post_session('sys_roles', $post);
	...
	function searchList()
	{
		...
		if ($post) {
			...
			$rows = $this->sys_roles_model->get_rows($post);
			$rows_publish = $this->sys_roles_model->get_rows_publish($post);
			$rows_modified = $this->sys_roles_model->get_rows_modified($post);
			$rows_unpublish = $this->sys_roles_model->get_rows_unpublish($post);
