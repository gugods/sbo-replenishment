...
class Sys_users extends Backend
{
	...
	function __construct()
	{
		...
		parent::$view_data["add_mode"] = "On";
		parent::$view_data["edit_mode"] = "On";
		parent::$view_data["publish_mode"] = "On";
		parent::$view_data["delete_mode"] = "On";
		parent::$view_data["export_mode"] = "Off";
		parent::$view_data["undelete_mode"] = "On";
	...
	function formList()
	{
		...
		$post = get_post_session('sys_users', $post);
		...
		parent::$view_data["sorting"] = isset($post['sorting']) ? $post['sorting']  : 'sys_users_id';
		...
		parent::$view_data["rows_delete"] = $this->sys_users_model->get_rows_delete($post);
	...
	function ajaxList()
	{
		...
		if ($post) {
			set_post_session('sys_users', $post);
	...
	function searchList()
	{
		...
		if ($post) {
			...
			$rows = $this->sys_users_model->get_rows($post);
			$rows_publish = $this->sys_users_model->get_rows_publish($post);
			$rows_modified = $this->sys_users_model->get_rows_modified($post);
			$rows_unpublish = $this->sys_users_model->get_rows_unpublish($post);
			$rows_delete = $this->sys_users_model->get_rows_delete($post);
	...
	private function cmdUnDelete()
	{
		...
		if (gettype($post["id"]) == "array") {
			foreach ($post["id"] as $key => $val) {
				$status = self::trigger("undelete:before", $val, array());
				if ($status) {
					$this->sys_logs_model->save_log($this->module, "undelete", $val);
					$this->sys_users_model->undelete($val);
					$status = self::trigger("undelete:after", $val, array());
				} else {
					break;
				}
			}
	...
	function cmdUnDeleteToList()
	{
		$post = $this->input->post();
		if ($post) {
			$status = self::cmdUnDelete();
			if ($status) {
				$alert_message = isset($post['username']) && gettype($post['username']) !== 'array' ? '"' . $post['username'] . '"' : "";
				$this->session->set_flashdata('ALERT_MESSAGE', 'You undelete ' . $alert_message . ' completed.');
			}
			$site_url = site_url("positions");
			$result = array(
				"status" => $status, "message" => self::$message, "url" => $site_url
			);
	...
	function cmdDeleteFile()
	{
		...
		if ($post) {
			...
			$this->sys_users_model->delete_file($post);
