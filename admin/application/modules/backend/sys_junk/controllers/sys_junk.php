<?php
class Sys_junk extends Backend
{
	...
	function __construct()
	{
		...
		$this->load->model("sys_junk_model");
	...
	function formList()
	{
		...
		parent::$view_data["rows_publish"] = $this->sys_junk_model->get_rows_publish($post);
		parent::$view_data["rows_modified"] = $this->sys_junk_model->get_rows_modified($post);
		parent::$view_data["rows_unpublish"] = $this->sys_junk_model->get_rows_unpublish($post);
		parent::$view_data["totalpage"] = ceil($rows / parent::$view_data["pagesize"]);
		parent::$view_data["post"] = $post;
		parent::$view_data["control"] = true;

		$this->load->view("sys_junk_list_view", parent::$view_data);
	}
	...
	function ajaxList()
	{
		...
		if ($post) {
			set_post_session('sys_junk', $post);
			...
			$start = $post["thispage"];
			$end = $post["pagesize"];
			$item = $this->sys_junk_model->get_page($option, $start, $end);
	...
	function searchList()
	{
		...
		if ($post) {
			...
			$rows = $this->sys_junk_model->get_rows($post);
			$rows_publish = $this->sys_junk_model->get_rows_publish($post);
			$rows_modified = $this->sys_junk_model->get_rows_modified($post);
			$rows_unpublish = $this->sys_junk_model->get_rows_unpublish($post);
	...
	function formDisplay($id = 0)
	{
		...
		$this->sys_junk_model->count_log($id, $rows, $totalpage);
		parent::$view_data["rows"] = $rows;
		...
		if ($status) {
			...
			if ($status) {
				...
				if ($status) {
					...
					$this->sys_junk_model->savePublish($val, $cmd);
		...
		if (gettype($post["id"]) == "array") {
			foreach ($post["id"] as $key => $val) {
				...
				if ($status) {
					...
					$this->sys_junk_model->delete($val);
	...
	private function cmdPublish(&$id)
	{
		...
		if (gettype($post["id"]) == "array") {
			foreach ($post["id"] as $key => $val) {
				...
				if ($status) {
					$this->sys_logs_model->save_log($this->module, "publish", $val);
					$this->sys_junk_model->save_public($val);
	...
	private function cmdUnPublish()
	{
		...
		foreach ($post["id"] as $key => $val) {
			...
			if ($status) {
				...
				$this->sys_junk_model->save_unpublic($val);
	...
	function cmdSaveToForm()
	{
		...
		if ($post) {
			...
			$site_url = site_url("sys_junk/formEdit/" . $id);
			$result = array(
				"status" => $status, "message" => self::$message, "url" => $site_url
			);
	...
	function cmdDeleteFile()
	{
		...
		if ($post) {
			...
			@unlink($BASEPATH . "upload/sys_junk/thumb_edit/" . @$post['filepath']);
			@unlink($BASEPATH . "upload/sys_junk/thumb_list/" . @$post['filepath']);
			@unlink($BASEPATH . "upload/sys_junk/" . @$post['filepath']);

			$this->sys_junk_model->delete_file($post);

			$site_url = site_url("sys_junk/formEdit/" . @$post["id"]);
			$result = array(
	...
	function changeLog()
	{
		...
		if ($post) {
			...
			$start = $post["thispage"];
			$end = $post["pagesize"];
			$item = $this->sys_junk_model->get_log($option, $start, $end);
			echo  json_encode($item);
		} else {
			show_404();
		}
	...
	function cmdUpdateParent()
	{
		...
		if ($post) {
			...
			$this->sys_junk_model->update_parent($json_array, 0);
		} else {
			show_404();
		}
	}
	...
	function cmdUpdateStatus()
	{
		...
		if ($post) {
			...
			parent::check_access($this->module, "modified");
			$this->sys_junk_model->update_status($post);
		} else {
			show_404();
		}
	}
	...
	{
		$obj = modules::run('sys_junk/sys_junk_trigger/set_trigger', $action, $id, $data);
		self::$message = $obj["message"];
		return $obj["status"];
	}
}
