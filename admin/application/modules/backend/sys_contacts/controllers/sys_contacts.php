<?php
class Sys_contacts extends Backend
{
	public static $message = "";
	var $module = "sys_contacts";
	var $title = "Inbox";
	...
	function __construct()
	{
		...
		$this->load->model("sys_contacts_model");
	...
	function formList()
	{
		...
		$post = get_post_session('sys_contacts', $post);

		$status = self::trigger("view:list", null, $post);
		parent::check_access($this->module, "view");

		$rows = $this->sys_contacts_model->get_rows($post);
		parent::$view_data["thispage"] = isset($post['thispage']) ? $post['thispage']   : 1;
		parent::$view_data["pagesize"] = isset($post['pagesize']) ? $post['pagesize']  : 30;
		parent::$view_data["sys_action"] = isset($post['sys_action']) ? $post['sys_action'] : '';
		parent::$view_data["orderby"] = isset($post['orderby']) ? $post['orderby']  : 'desc';
		parent::$view_data["sorting"] = isset($post['sorting']) ? $post['sorting']  : 'sys_contacts_id';

		parent::$view_data["rows"] = $rows;
		parent::$view_data["rows_publish"] = $this->sys_contacts_model->get_rows_publish($post);
		parent::$view_data["rows_modified"] = $this->sys_contacts_model->get_rows_modified($post);
		parent::$view_data["rows_unpublish"] = $this->sys_contacts_model->get_rows_unpublish($post);
		parent::$view_data["totalpage"] = ceil($rows / parent::$view_data["pagesize"]);
		parent::$view_data["post"] = $post;
		parent::$view_data["control"] = true;

		$this->load->view("sys_contacts_list_view", parent::$view_data);
	}
	...
	function ajaxList()
	{
		...
		if ($post) {
			set_post_session('sys_contacts', $post);
			...
			$start = $post["thispage"];
			$end = $post["pagesize"];
			$item = $this->sys_contacts_model->get_page($option, $start, $end);
	...
	function searchList()
	{
		...
		if ($post) {
			...
			$rows = $this->sys_contacts_model->get_rows($post);
			$rows_publish = $this->sys_contacts_model->get_rows_publish($post);
			$rows_modified = $this->sys_contacts_model->get_rows_modified($post);
			$rows_unpublish = $this->sys_contacts_model->get_rows_unpublish($post);
	...
	function formDisplay($id = 0)
	{
		...
		$this->sys_contacts_model->count_log($id, $rows, $totalpage);
		parent::$view_data["rows"] = $rows;
	...
	private function cmdSave(&$id)
	{
		...
		if ($status) {
			if (empty($val)) {
				parent::check_access($this->module, "created");
				$val = $this->sys_contacts_model->insert($post);
				$this->sys_logs_model->save_log($this->module, "created", $val);
			} else {
				parent::check_access($this->module, "modified");
				$this->sys_logs_model->save_log($this->module, "modified", $val);
				$this->sys_contacts_model->update($post);
			}
	...
	private function cmdSavePublish(&$id)
	{
		...
		if ($status) {
			...
			if ($status) {
				...
				if ($status) {
					$this->sys_logs_model->save_log($this->module, "publish", $val);
					$this->sys_contacts_model->savePublish($val, $cmd);
	...
	private function cmdDelete()
	{
		...
		if (gettype($post["id"]) == "array") {
			foreach ($post["id"] as $key => $val) {
				...
				if ($status) {
					$this->sys_logs_model->save_log($this->module, "delete", $val);
					$this->sys_contacts_model->delete($val);
	...
	private function cmdPublish(&$id)
	{
		...
		if (gettype($post["id"]) == "array") {
			foreach ($post["id"] as $key => $val) {
				...
				if ($status) {
					$this->sys_logs_model->save_log($this->module, "publish", $val);
					$this->sys_contacts_model->save_public($val);
	...
	private function cmdUnPublish()
	{
		...
		foreach ($post["id"] as $key => $val) {
			...
			if ($status) {
				$this->sys_logs_model->save_log($this->module, "unpublish", $val);
				$this->sys_contacts_model->save_unpublic($val);
	...
	function cmdSaveToList()
	{
		...
		if ($post) {
			...
			$site_url = site_url("sys_contacts");

			$result = array(
				"status" => $status, "message" => self::$message, "url" => $site_url
			);
	...
	function cmdSaveToForm()
	{
		...
		if ($post) {
			...
			$site_url = site_url("sys_contacts/formEdit/" . $id);
			$result = array(
				"status" => $status, "message" => self::$message, "url" => $site_url
			);
	...
	function cmdDeleteFile()
	{
		...
		if ($post) {
			...
			@unlink($BASEPATH . "upload/sys_contacts/thumb_edit/" . @$post['filepath']);
			@unlink($BASEPATH . "upload/sys_contacts/thumb_list/" . @$post['filepath']);
			@unlink($BASEPATH . "upload/sys_contacts/" . @$post['filepath']);

			$this->sys_contacts_model->delete_file($post);

			$site_url = site_url("sys_contacts/formEdit/" . @$post["id"]);
			$result = array(
				"status" => true, "message" => self::$message, "url" => $site_url
			);
	...
	function changeLog()
	{
		...
		if ($post) {
			...
			$start = $post["thispage"];
			$end = $post["pagesize"];
			$item = $this->sys_contacts_model->get_log($option, $start, $end);
			echo  json_encode($item);
		} else {
			show_404();
		}
	...
	function cmdAction()
	{
		...
		$result = array(
			"status" => $status, "message" => self::$message
		);
	...
	function cmdUpdateParent()
	{
		...
		if ($post) {
			...
			$this->sys_contacts_model->update_parent($json_array, 0);
		} else {
			show_404();
		}
	}
	...
	function cmdUpdateStatus()
	{
		...
		if ($post) {
			...
			parent::check_access($this->module, "modified");
			$this->sys_contacts_model->update_status($post);
		} else {
			show_404();
		}
	}
	...
	{
		$obj = modules::run('sys_contacts/sys_contacts_trigger/set_trigger', $action, $id, $data);
		self::$message = $obj["message"];
		return $obj["status"];
	}
}
