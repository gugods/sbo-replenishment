...
function get_page($sql, $driver, $start, $end)
{
	...
	switch ($driver):
		case "mysqli":
			$listdata = mysql_new_query($sql, $start, $end);
			break;
		case "mysql":
			$listdata = mysql_new_query($sql, $start, $end);
			break;
		case "mssql":
			$listdata = mssql_new_query($sql, $start, $end);
			break;
		case "sqlsrv":
			$listdata = mssql_new_query($sql, $start, $end);
			break;
		case "oci8":
			$listdata = oci8_new_query($sql, $start, $end);
			break;
	endswitch;
	return $listdata;
}
...
function getWhereOrFindInSet($fields = [], $field_name = '')
{
	$where = '(';
	foreach ($fields as $key => $val) {
		if ($key == 0) {
			$where .= " FIND_IN_SET(" . $val . ",{$field_name}) > 0 ";
		} else {
			$where .= " OR FIND_IN_SET(" . $val . ",{$field_name}) > 0 ";
		}
	}
	$where .= ')';
	return 	$where;
